...
def test_triples(p1: list[Triple], p2: list[Triple]):
  ...
  for i in range(len(p1)):
    ...
    y2 = (y1-random.randint(0, config.MPC_MODULO)) % config.MPC_MODULO
    ...
    if r != z:
      raise ValueError(f'({p1[i].a + p2[i].a}, {p1[i].b + p2[i].b}, {p1[i].c + p2[i].c} is a wrong triple!, id:{i}')


if __name__ == '__main__':
  p1_triples = load_triples('triples/p0.csv')
  p2_triples = load_triples('triples/p1.csv')
